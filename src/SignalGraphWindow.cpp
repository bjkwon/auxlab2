...
void SignalGraphWindow::paintEvent(QPaintEvent*) {
  ...
  QString xLabel = data_.isAudio ? "Time (s)" : "Index";
  QString yLabel = data_.isAudio ? "Amplitude [-1, 1]" : QString("Value [%1, %2]").arg(yMin_, 0, 'g', 5).arg(yMax_, 0, 'g', 5);
  ...
  if (selStart_ >= 0 && selEnd_ >= 0 && selStart_ != selEnd_) {
    ...
    int x1 = plot.left() + (s - viewStart_) * plot.width() / total;
    int x2 = plot.left() + (e - viewStart_) * plot.width() / total;
    p.fillRect(QRect(std::min(x1, x2), plot.top(), std::abs(x2 - x1), plot.height()), QColor(120, 170, 255, 60));
  }

  if (audioSink_ && audioSink_->state() != QAudio::StoppedState) {
    const int span = std::max(1, playingRange_.end - playingRange_.start);
    const qint64 elapsedMs = playheadClock_.elapsed();
    double frac = (elapsedMs / 1000.0) * data_.sampleRate / static_cast<double>(span);
    frac = std::clamp(frac, 0.0, 1.0);
    int sample = playingRange_.start + static_cast<int>(span * frac);
    sample = std::clamp(sample, viewStart_, viewStart_ + viewLen_ - 1);
    const int total = std::max(1, viewLen_ - 1);
    const int x = plot.left() + (sample - viewStart_) * plot.width() / total;
...
void SignalGraphWindow::startPlaybackForRange(const Range& range) {
  ...
  pcmData_.resize(frames * chCount * static_cast<int>(sizeof(float)));

  auto* out = reinterpret_cast<float*>(pcmData_.data());
  for (int i = start; i < end; ++i) {
    for (int c = 0; c < chCount; ++c) {
      const auto& src = data_.channels[static_cast<size_t>(c)].samples;
      float v = 0.0f;
      if (i < static_cast<int>(src.size())) {
        v = static_cast<float>(std::clamp(src[static_cast<size_t>(i)], -1.0, 1.0));
      }
      *out++ = v;
    }
  }
